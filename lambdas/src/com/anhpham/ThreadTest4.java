...
public class ThreadTest4 {
    ...
    private static void countPrimesWithExecutor(int numberOfTasks) {
        ...
        for (int i = 0; i < numberOfTasks; i++) {
            ...
            // FIXME: Convert oneTask to lambda
            int finalMin = min;
            int finalMax = max;

            Future<Integer> oneResult = executor.submit(() -> countPrimes(finalMin, finalMax));
        ...
        // FIXME: Convert iterate to lambda
        AtomicInteger total = new AtomicInteger();

        results.forEach((res) -> {
            try {
                total.addAndGet(res.get());  // Waits for task to complete!
            } catch (Exception e) {
                ...
                System.err.println("Error occurred while computing: " + e);
            }
        });
